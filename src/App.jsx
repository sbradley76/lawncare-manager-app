...
const INVOICE_PAYMENT_METHOD_OPTIONS = ['cash', 'check', 'cash_app', 'venmo', 'zelle', 'card', 'other'];
...
function invoiceStatusTone(status) {
  ...
  if (status === 'sent' || status === 'viewed' || status === 'partially_paid') return 'sent';
    ...
    <main className="app-grid">
      <section className="main-panel">
        ...
        <div className="stats-grid">
          ...
          <StatCard label="Unpaid Jobs" value={unpaidJobs} hint={`${scheduledJobs} scheduled jobs`} />
...
function Routes({ jobs, invoices, invoiceItems, settings, refreshData }) {
  ...
  async function createInvoiceFromJob(job) {
    ...
    setSavingId(job.id);
    const amountCents = Number(job.final_price_cents || job.quoted_price_cents || 0);
    const serviceDate = job.scheduled_date || new Date().toISOString().slice(0, 10);
    const due = new Date(`${serviceDate}T12:00:00`);
    ...
    const description = normalizeServices(job.services).map(labelize).join(', ') || 'Lawn care service';
    ...
    setSavingId(null);
    if (itemError || jobError) alert((itemError || jobError).message);
    await refreshData(false);
  }
              ...
              <article className="job-card" key={job.id}>
                ...
                <div className="invoice-mini-panel">
                  {invoice ? (
                    ...
                    <button className="secondary-button full" onClick={() => createInvoiceFromJob(job)} disabled={savingId === job.id}>
                      Generate invoice
                    </button>
                  )}
                </div>
...
function InvoiceDetail({ invoice, items, settings, refreshData, jobs }) {
  ...
  async function recalcTotals(nextItems = items, patch = {}) {
    ...
    const { error } = await supabase
      .from('lawncare_invoices')
      .update({ subtotal_cents: subtotal, total_cents: totalCents, ...patch })
      .eq('id', invoice.id);
    return error;
  }
  ...
  async function saveInvoice() {
    setBusy(true);
    setMessage('');
    const paidCents = amountPaid === '' ? 0 : dollarsToCents(amountPaid);
    const finalStatus = status === 'paid' && !amountPaid ? 'paid' : status;
    const patch = {
      status: finalStatus,
      payment_method: paymentMethod || null,
      amount_paid_cents: paidCents ?? 0,
    ...
    if (!error && linkedJob && finalStatus === 'paid') {
      await supabase.from('lawncare_jobs').update({ payment_status: 'paid_card' }).eq('id', linkedJob.id);
    }
    setBusy(false);
  ...
  async function addItem(event) {
    ...
    if (!error) await recalcTotals([...items, data]);
    setBusy(false);
    if (error) setMessage(error.message);
    else {
      await refreshData(false);
  ...
  async function deleteItem(item) {
    ...
    if (!error) await recalcTotals(items.filter((current) => current.id !== item.id));
    setBusy(false);
    if (error) setMessage(error.message);
        ...
        <div className="invoice-items">
          {items.length ? items.map((item) => (
            <div className="invoice-item" key={item.id}>
              ...
              <strong>{centsToDollars(item.line_total_cents)}</strong>
              <button className="tiny-button" onClick={() => deleteItem(item)} disabled={busy}>Remove</button>
            </div>
          )) : <p className="small-muted">No line items yet.</p>}
        </div>
        <form className="add-item-row" onSubmit={addItem}>
          <input value={newDescription} onChange={(event) => setNewDescription(event.target.value)} placeholder="Add service or fee" />
          <input value={newPrice} onChange={(event) => setNewPrice(event.target.value)} placeholder="Price" inputMode="decimal" />
        ...
        <div className="two-col">
          ...
          <label>Payment method
            <select value={paymentMethod} onChange={(event) => setPaymentMethod(event.target.value)}>
              ...
              {INVOICE_PAYMENT_METHOD_OPTIONS.map((option) => <option key={option} value={option}>{labelize(option)}</option>)}
            </select>
          </label>
        </div>
        <label>Amount paid
          <input value={amountPaid} onChange={(event) => setAmountPaid(event.target.value)} placeholder="0.00" inputMode="decimal" />
        </label>
        <label>Invoice notes
          <textarea value={notes} onChange={(event) => setNotes(event.target.value)} placeholder="Private payment or customer notes" />
        </label>
        <div className="action-row wrap">
          <button className="primary-button compact" onClick={saveInvoice} disabled={busy}>Save invoice</button>
          <button className="secondary-button compact" onClick={markPaid} disabled={busy}>Mark paid</button>
          <button className="ghost-button compact" onClick={copyInvoiceText}>Copy invoice text</button>
          <button className="ghost-button compact" onClick={() => window.print()}>Print</button>
        </div>
      </section>
    ...
    <main className="app-grid invoices-grid">
      <section className="main-panel">
        ...
        <div className="stats-grid">
          ...
          <StatCard label="Open" value={openCount} hint="Need payment/follow-up" />
          <StatCard label="Paid" value={paidCount} hint="Closed invoices" />
          <StatCard label="Open balance" value={centsToDollars(openTotal)} hint="Not paid or void" />
        </div>
        ...
        <div className="lead-list">
          ...
          }) : <EmptyState title="No invoices yet" body="Complete a job and generate an invoice from the Routes page." />}
        </div>
      </section>
...
export default function App() {
  ...
  async function refreshData(showSpinner = true) {
    ...
    const [leadsResponse, quotesResponse, jobsResponse, settingsResponse, imagesResponse, invoicesResponse, invoiceItemsResponse] = await Promise.all([
  ...
  return (
    <div className="manager-app">
      ...
      {activeView === 'routes' ? <Routes jobs={jobs} invoices={invoices} invoiceItems={invoiceItems} settings={settings} refreshData={refreshData} /> : null}
      {activeView === 'invoices' ? <Invoices invoices={invoices} invoiceItems={invoiceItems} jobs={jobs} settings={settings} refreshData={refreshData} /> : null}
